...
/**
 * API route for serving diagram files (SVG) from portfolio-data/diagrams
 *
 * Endpoint: /portfolio-data/diagrams/[filename]
 * Method: GET
 *
 * Features:
...
export async function GET(
  ...
  try {
    ...
    if (path.isAbsolute(configPath)) {
      ...
      diagramPath = path.join(configPath, 'diagrams', filename);
    } else {
      // Relative path (local development)
      const projectRoot = path.resolve(process.cwd(), '../../..');
      diagramPath = path.resolve(projectRoot, configPath, 'diagrams', filename);
    }
