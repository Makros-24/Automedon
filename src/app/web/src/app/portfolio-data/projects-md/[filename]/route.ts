...
/**
 * API route for serving markdown files from portfolio-data/projects-md
 *
 * Endpoint: /portfolio-data/projects-md/[filename]
 * Method: GET
 *
 * Features:
 * - Server-side markdown file loading
...
export async function GET(
  ...
  try {
    ...
    if (path.isAbsolute(configPath)) {
      ...
      markdownPath = path.join(configPath, 'projects-md', filename);
    } else {
      // Relative path (local development)
      const projectRoot = path.resolve(process.cwd(), '../../..');
      markdownPath = path.resolve(projectRoot, configPath, 'projects-md', filename);
    }
